...
export class InitialWashFlowSchema1713030000000 implements MigrationInterface {
  name = 'InitialWashFlowSchema1713030000000';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."services_category_enum" AS ENUM('wash', 'dry_clean', 'iron', 'wash_iron')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."orders_status_enum" AS ENUM('requested', 'accepted', 'picked_up', 'cleaning', 'out_for_delivery', 'delivered', 'cancelled')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."orders_payment_status_enum" AS ENUM('pending', 'paid', 'refunded')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."order_items_category_enum" AS ENUM('wash', 'dry_clean', 'iron', 'wash_iron')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."subscriptions_plan_name_enum" AS ENUM('silver', 'gold', 'platinum')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."coupons_discount_type_enum" AS ENUM('percentage', 'flat')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."notifications_type_enum" AS ENUM('order', 'promo', 'system')`,
    );
